...
va_list vap)
{
    extern HSim__s6 * createworkMnand__wf(const char*);
    HSim__s6 *blk = createworkMnand__wf(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMnand__sl(const char*);
    HSim__s6 *blk = createworkMnand__sl(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","nand_wf", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","nand_sl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMnand__wf(const char*);
        topvl = (HSim__s5*)createworkMnand__wf("nand_wf");
