...
va_list vap)
{
    extern HSim__s6 * createworkMcorrelation__wf(const char*);
    HSim__s6 *blk = createworkMcorrelation__wf(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMcorrelation(const char*);
    HSim__s6 *blk = createworkMcorrelation(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","correlation_wf", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","correlation", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMcorrelation__wf(const char*);
        topvl = (HSim__s5*)createworkMcorrelation__wf("correlation_wf");
