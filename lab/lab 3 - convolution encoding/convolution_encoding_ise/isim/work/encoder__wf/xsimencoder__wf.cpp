...
va_list vap)
{
    extern HSim__s6 * createworkMencoder__wf(const char*);
    HSim__s6 *blk = createworkMencoder__wf(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","encoder_wf", (HSimInstFactoryPtr)IF0);
        ...
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMencoder__wf(const char*);
        topvl = (HSim__s5*)createworkMencoder__wf("encoder_wf");
