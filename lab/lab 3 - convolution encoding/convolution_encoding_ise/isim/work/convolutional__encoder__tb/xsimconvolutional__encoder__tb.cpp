...
va_list vap)
{
    extern HSim__s6 * createworkMconvolutional__encoder__tb(const char*);
    HSim__s6 *blk = createworkMconvolutional__encoder__tb(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","convolutional_encoder_tb", (HSimInstFactoryPtr)IF0);
        ...
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMconvolutional__encoder__tb(const char*);
        topvl = (HSim__s5*)createworkMconvolutional__encoder__tb("convolutional_encoder_tb");
