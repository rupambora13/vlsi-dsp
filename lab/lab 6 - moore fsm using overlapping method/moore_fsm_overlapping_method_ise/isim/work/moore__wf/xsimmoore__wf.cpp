...
va_list vap)
{
    extern HSim__s6 * createworkMmoore__wf(const char*);
    HSim__s6 *blk = createworkMmoore__wf(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMmoore__fsm(const char*);
    HSim__s6 *blk = createworkMmoore__fsm(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","moore_wf", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","moore_fsm", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMmoore__wf(const char*);
        topvl = (HSim__s5*)createworkMmoore__wf("moore_wf");
