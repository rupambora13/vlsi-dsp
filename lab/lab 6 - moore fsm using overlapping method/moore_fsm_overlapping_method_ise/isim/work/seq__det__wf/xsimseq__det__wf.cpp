...
va_list vap)
{
    extern HSim__s6 * createworkMseq__det__wf(const char*);
    HSim__s6 *blk = createworkMseq__det__wf(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMseq__detector(const char*);
    HSim__s6 *blk = createworkMseq__detector(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","seq_det_wf", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","seq_detector", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMseq__det__wf(const char*);
        topvl = (HSim__s5*)createworkMseq__det__wf("seq_det_wf");
