...
va_list vap)
{
    extern HSim__s6 * createworkMfir__wf(const char*);
    HSim__s6 *blk = createworkMfir__wf(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMfir__filter(const char*);
    HSim__s6 *blk = createworkMfir__filter(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","fir_wf", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","fir_filter", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMfir__wf(const char*);
        topvl = (HSim__s5*)createworkMfir__wf("fir_wf");
