...
va_list vap)
{
    extern HSim__s6 * createworkMconvolution__wf(const char*);
    HSim__s6 *blk = createworkMconvolution__wf(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMconvolution(const char*);
    HSim__s6 *blk = createworkMconvolution(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","convolution_wf", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","convolution", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMconvolution__wf(const char*);
        topvl = (HSim__s5*)createworkMconvolution__wf("convolution_wf");
