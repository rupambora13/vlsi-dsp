...
va_list vap)
{
    extern HSim__s6 * createworkMma__process(const char*);
    HSim__s6 *blk = createworkMma__process(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","ma_process", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMma__process(const char*);
        topvl = (HSim__s5*)createworkMma__process("ma_process");
