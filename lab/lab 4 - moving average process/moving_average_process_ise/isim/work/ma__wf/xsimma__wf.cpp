...
va_list vap)
{
    extern HSim__s6 * createworkMma__wf(const char*);
    HSim__s6 *blk = createworkMma__wf(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMmoving__average__filter(const char*);
    HSim__s6 *blk = createworkMmoving__average__filter(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","ma_wf", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","moving_average_filter", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMma__wf(const char*);
        topvl = (HSim__s5*)createworkMma__wf("ma_wf");
